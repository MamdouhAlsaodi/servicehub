...
export default function AdminPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [k, r, t, p, d] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/kpis`, init),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/reports/revenue?days=30`, init),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/reports/top-vendors?limit=5`, init),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/vendors/pending`, init),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/disputes`, init),
      ]);
      if (k.status === 401) {
      ...
      if (k.status === 403) {
        setError("هذه الصفحة للمدير فقط");
        setLoading(false);
        return;
      }
      if (k.ok) setKpis(await k.json());
      if (r.ok) setRevenue(await r.json());
      if (t.ok) setTopVendors(await t.json());
  ...
  async function approve(id: string) {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/admin/vendors/${id}/approve`,
      {
        method: "PATCH",
        credentials: "include",
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        {kpis && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <KpiCard
              ...
              tone="emerald"
              sub={`عمولة: ${fmtBRL(kpis.commission)}`}
            />
          </div>
        )}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold">الإيرادات — آخر 30 يوم</h2>
            <span
          ...
          <div className="flex items-center justify-between mt-3 text-[10px] opacity-50">
            ...
            <span>
              الإجمالي: {fmtBRL(revenue.reduce((s, r) => s + r.amount, 0))}
            </span>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold">أفضل البائعين</h2>
            ...
            {topVendors.length === 0 ? (
              <p className="text-xs opacity-50 text-center py-6">لا توجد بيانات</p>
            ) : (
          ...
          <section
            ...
            style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
          >
            <h2 className="text-sm font-bold mb-4">بانتظار الموافقة</h2>
            {pending.length === 0 ? (
              <p className="text-xs opacity-50 text-center py-6">
                لا يوجد بائعون بانتظار الموافقة ✨
              </p>
            ) : (
              <div className="space-y-2">
                {pending.map((v) => (
                  ...
                  >
                    ...
                    >
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> موافقة
                    </button>
                  </div>
                ))}
              </div>
        ...
        <section
          ...
          style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
        >
          <h2 className="text-sm font-bold mb-4">إلغاءات تستحق المراجعة</h2>
          {disputes.length === 0 ? (
            <p className="text-xs opacity-50 text-center py-6">لا توجد نزاعات ✨</p>
          ) : (
            <div className="space-y-2">
              {disputes.map((d) => (
                <div
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] opacity-60 truncate">
                      {d.customer.name} @ {d.vendor.businessName} · {new Date(d.startTime).toLocaleDateString("ar")}
                    </p>
                    <p className="text-[11px] mt-1 opacity-70" dir="rtl">
                      السبب: {d.cancellationReason}
                    </p>
                  </div>
                </div>
              ))}
